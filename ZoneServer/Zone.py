...
class Zone(Server):
    ...
    def transfer_world(self, session, args):
        ...
        Packets.Outgoing.TRANSFER_TO_WORLD.TRANSFER_TO_WORLD(stream=None, conn=conn, server=self, is_transfer=True,
                                                             zone_id=args[1])

    def kick(self, conn, server):
        address = (str(conn.get_address()[0]), int(conn.get_address()[1]))
        uid = str(uuid3(NAMESPACE_DNS, str(address)))
    ...
    def fly(self, session, filler_argument):
        ...
        if flight_mode is False:
            ...
            message = SetJetPackMode.SetJetPackMode(objid=session.current_character.object_id, bypass_checks=True,
                                                    use=True, effect_id=-1)
            ...
            message = SetJetPackMode.SetJetPackMode(objid=session.current_character.object_id, bypass_checks=False,
                                                    use=False, effect_id=-1)
...
class CustomReplicaManager(ReplicaManager):
    ...
    def _construct(self, obj: Replica, new: bool = True, important: bool = False,
                   recipients: Iterable[Connection] = None, reliability=None) -> None:
