...
class Zone(Server):
	...
	def _on_new_conn(self, data: ReadStream, conn: Connection) -> None:
		log(LOGGINGLEVEL.WORLDDEBUG, (" New Connection from " + conn.get_address()[0] + ":" + str(conn.get_address()[1])))
	...
	def _on_lu_packet(self, data: bytes, conn: Connection):
		...
		try:
			...
			log(LOGGINGLEVEL.WORLDDEBUG, " Handled", identifier)
		except KeyError:
			log(LOGGINGLEVEL.WORLDDEBUG, " Unhandled", identifier)
